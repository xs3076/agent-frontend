...
export interface IEndNodeParam {
  ...
  /* Output type: json (output json), text (output text), none (no output) */
  output_type: 'json' | 'text' | 'none';
...
export interface IChartNodeParam {
  ...
  option_override?: Record<string, unknown>;
  display?: {
    title?: string;
    width?: string;
    height?: number;
...
export type IChartNodeData = IWorkFlowNodeData<IChartNodeParam>;
